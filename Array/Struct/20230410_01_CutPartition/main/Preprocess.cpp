...
template <class Type, class Size> Size multiPivotSplit_Array(Array<Type, Size>& array, Size groupMumber=5, Size groupOrder=0xffffffffUL, Size order=0xffffffffUL)
  {
    ...
    for (Size g=0; g<pivotArray.length; g++)
      {
        ...
        std::qsort((array.start+(groupMumber*g)), g_length, sizeof(Type), [](const void* a, const void* b)->int{return *(Type*)a - *(Type*)b;});
        *(pivotArray.start+g) = *(array.start+(groupMumber*g)+(g_length%groupOrder?groupOrder:(g_length-1)));
      }
    std::qsort(pivotArray.start, pivotArray.length, sizeof(Type), [](const void* a, const void* b)->int{return *(Type*)a - *(Type*)b;});
...
int main()
  {
    ...
    size_t groupMumber; std::cout<<"groupMumber: "; std::cin>>groupMumber;
    std::cout<<std::endl; pivot = multiPivotSplit_Array(Data, groupMumber);
    std::cout<<Data<<std::endl;
    std::cout<<"pivot index: "<<pivot<<std::endl;
    std::cout<<"pivot value: "<<*(Data.start+pivot)<<std::endl;
  }
